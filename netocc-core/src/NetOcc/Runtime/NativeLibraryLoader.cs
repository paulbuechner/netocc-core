...
/// <summary>
/// Loads the NetOcc native libraries (NetOcc&lt;Module&gt; with the wrappers of an OCCT module, NetOccRuntime) and, through
/// the OS loader, the OCCT libraries next to them, before the first P/Invoke. Probed locations, relative to the app base
/// directory: <c>&lt;arch&gt;/</c> (AnyCPU .NET Framework layout), <c>runtimes/&lt;rid&gt;/native/</c>, and the base directory.
/// </summary>
...
#pragma warning restore CA2255
    internal static void Initialize()
    {
        ...
        NativeLibrary.SetDllImportResolver(typeof(NativeLibraryLoader).Assembly, static (name, _, _) =>
            name.StartsWith(Prefix, StringComparison.Ordinal) ? Load(name) : IntPtr.Zero);
#else
        // .NET Framework: DllImport resolves by module name, so preloaded modules win: every NetOcc library found first
        if (IsWindows)
        {
            foreach (var directory in Directories())
            {
                if (Directory.Exists(directory) && Directory.GetFiles(directory, $"{Prefix}*.dll") is { Length: > 0 } libraries)
                {
                    foreach (var library in libraries)
                    {
                        Load(Path.GetFileNameWithoutExtension(library));
                    }
    ...
    private static IntPtr Load(string name)
    {
        lock (Sync)
        {
            ...
            foreach (var directory in Directories())
            {
                ...
                handle = IsWindows ? LoadLibraryExW(path, IntPtr.Zero, LoadWithAlteredSearchPath) : LoadUnix(path);
            ...
            // IntPtr.Zero: fall back to default probing
            Handles[name] = handle;
            return handle;
        }
    }

    // where the natives may be, in probing order
    private static string[] Directories()
    {
        ...
        return
        [
            Combine(baseDir, arch),
            Combine(baseDir, "runtimes", $"{OsName()}-{arch}", "native"),
            baseDir,
        ];
    }
