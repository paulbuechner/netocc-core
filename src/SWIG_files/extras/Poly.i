...
%extend Poly_Triangulation {
  void CopyNodesTo(gp_Pnt* NETOCC_ARRAY) const {
    const int n = $self->NbNodes();
    for (int i = 1; i <= n; ++i) NETOCC_ARRAY[i - 1] = $self->Node(i);
  }
  void CopyTrianglesTo(Poly_Triangle* NETOCC_ARRAY) const {
    const int n = $self->NbTriangles();
    for (int i = 1; i <= n; ++i) NETOCC_ARRAY[i - 1] = $self->Triangle(i);
  }
  %proxycode %{
